...
async def start(client, message):
    # Deep link support: search for movie name
    if len(message.command) > 1:
        query = message.command[1]
        from plugins.gfilters import search_query
        results = await search_query(client, query)
        if results:
            for result in results:
                try:
                    await message.reply_cached_media(result)
                except Exception as e:
                    await message.reply_text(f"⚠️ Error sending media: {e}")
        else:
